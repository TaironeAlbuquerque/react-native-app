...
const CreateEmployee = ({ navigation, route }) => {
  const getDetails = (type) => {
    if (route.params) {
      switch (type) {
        ...
        case 'tax':
          return route.params.tax;
        case 'picture':
          return route.params.picture;
        case 'hierarchy':
          return route.params.hierarchy;
      }
    }
    return '';
  };
  ...
  const [tax, setTax] = useState(getDetails('tax'));
  const [picture, setPicture] = useState(getDetails('picture'));
  const [hierarchy, setHierarchy] = useState(getDetails('hierarchy'));
  const [modal, setModal] = useState(false);
  const [enableshift, setenableShift] = useState(false);

  const submitData = () => {
    fetch('http://172.26.39.181:3000/send-data', {
      method: 'post',
      headers: {
  ...
  const updateDetails = () => {
    fetch('http://172.26.39.181:3000/update', {
      method: 'post',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    >
      <View>
        ...
        <TextInput
          ...
          onFocus={() => setenableShift(true)}
          mode="outlined"
          onChangeText={(text) => setHierarchy(text)}
        />
        <Button
          style={styles.inputStyle}
